...
2. Найти первый незавершенный раздел `### Task N:` или `### Iteration N:`.
...
- краткие `Overview` и `Context`;
- 2-6 независимых `### Task N:` разделов;
- конкретные чекбоксы `- [ ] ...`;
...
Каждый `### Task N:` должен быть независимо коммитабельным. Если задача просит
